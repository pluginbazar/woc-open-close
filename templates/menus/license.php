...
if( !empty( $nonce ) && wp_verify_nonce($nonce, 'woc_license_nonce_check') ) {
	...
	if( ! empty($_POST['woc_license_hidden']) && $_POST['woc_license_hidden'] == 'Y' ):
		...
		if( $pb_license_action == 'slm_activate' ):
			...
			if (is_wp_error($response)){
				printf( '<div class="%1$s"><p>%2$s</p></div>', 'notice notice-error is-dismissible', "Unexpected Error! The query returned with an error." ); 
			}
...
<div class="wrap woc_license">
	<h2>WooCommerce Open Close - <?php echo __('License', WOCTEXTDOMAIN); ?> </h2><br>
	<div class="pb_header_menu">
		<a class="active" href="edit.php?post_type=woc_hour&page=woc_menu_license">WooCommerce Open Close</a>
		<a class="" href="edit.php?post_type=woc_hour&page=woc_menu_wocr_license">WooCommerce Open Close - Reports</a>
	</div>
	<form  method="post" class="pb_form" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
		<input type="hidden" name="woc_license_hidden" value="Y" />
		...
		<div class="pb_section">
			...
			<div class="pb_section_info">License Key for <a href="" target="_blank"><b>WooCommerce Open Close</b></a></div>
